...
int  ExtractNodesFromPositionContext (const std::string s) 
{    
    std::smatch match; int sourceNode;
    std::regex rgx_srcNode (".*/NodeList/(\\w+)/*");
    if (std::regex_search(s.begin(), s.end(), match, rgx_srcNode))
        sourceNode = stoi(match[1]);
    return sourceNode;
  ...
  for (auto j = nodes.Begin (); j != nodes.End (); ++j)
    {
      ...
      uint32_t nodeId = object->GetId ();
      if (nodeName.empty ()) {
          nodeName = std::to_string (nodeId);
        }
                
      NS_LOG_UNCOND ("\t\t-------------");
      NS_LOG_UNCOND ("\t\tnodeName = " << nodeName << " (nodeId: "<< nodeId <<")");
      NS_LOG_UNCOND ("\t\t-----------------------------");
