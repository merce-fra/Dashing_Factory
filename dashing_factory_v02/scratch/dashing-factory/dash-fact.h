 ...
 **/
   ...
   * @param centralFrequencyBand The band central frequency ... etc
   */
     void Run ( double hBS, double hUT, int users, uint16_t numerology, double centralFrequencyBand, 
                ...
                uint8_t scenario, std::string indoorScenario, uint32_t seed);
  ...
  ns3::NodeContainer
  agv_positions_random (int ueHeight, const NodeContainer gnbNodes,
                       const NodeContainer agvNodes, double minDistance);

  ns3::NodeContainer
  agv_SrcDst_random    (int ueHeight, const NodeContainer gnbNodes,
                       const NodeContainer agvNodes, double minDistance, double simulationTime);
  ...
  // void PositionTracer (std::string context, Ptr<const MobilityModel> model);
  // void PositionTracer (Ptr<Node> node, std::ofstream csvFile);
  void PositionTracer (Ptr<Node> node);  

  void ChannelTracer (NodeContainer agvNodes, NodeContainer enbNodes);

  void RtoTracer (std::string context, Time oldval, Time newval);
  void TraceRto ();
  void Initialize_RtxTraceFile ();
  // void Initialize_PosTraceFile ();
  // void Initialize_outSinrFile ();
  // void Initialize_couplingTraceFile ();
  // void Initialize_outRssiFile ();
  void Initialize_someTraceFiles ();
  ...
  void AgvReception (RxPacketTraceParams params);
  void UeRssiPerProcessedChunk (double rssidBm);
  void AgvRssiPerProcessedChunk (double rssidBm);
  void AgvSnrPerProcessedChunk (double snrdBm);
  void SnrPerProcessedChunk (double snrdBm);

  uint8_t m_loggingLevel;
  uint32_t m_simTag;
  std::string m_scenarioTag;
...
private:
  ...
  std::ofstream m_agv_outSinrFile;     //!< the output file stream for the SINR file (agvz only)
  std::ofstream m_outSnrFile;          //!< the output file stream for the SNR file
  std::ofstream m_agv_outSnrFile;          //!< the output file stream for the SNR file
  std::ofstream m_outRssiFile;         //!< the output file stream for the RSSI file
  std::ofstream m_agv_outRssiFile;         //!< the output file stream for the RSSI file (agvz only)
  std::ofstream m_outCouplingFile;     //!< the output file stream for the coupling loss
  std::ofstream m_agv_outCouplingFile; //!< the output file stream for the coupling loss (agvz only)
  std::ofstream m_outPositionsFile;  //!< the output file stream for the UE positions file
  std::ofstream m_agv_outPositionsFile;  //!< the output file stream for the Agv positions file  
  std::ofstream m_gnb_outPositionsFile; //!< the output file stream for the gNB positions file
  ...
  Ptr<ChannelConditionModel> m_condModel; //!< the ChannelConditionModel object
  Ptr<ThreeGppPropagationLossModel> m_shadowingEnabled;  
