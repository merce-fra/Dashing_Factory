...
NrBearerStatsCalculator::UlTxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, uint32_t packetSize)
{
  ...
  if (!m_ulOutFile.is_open ())
    {
      ...
      m_ulOutFile << "protocol context timestamp imsi cellId rnti lcid packetSize " << std::endl;
    }
  ...
  m_ulOutFile << "UlTxPDU " << Simulator::Now () << " " << imsi << " " << cellId << " "
...
NrBearerStatsCalculator::DlTxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, uint32_t packetSize)
{
  ...
  if (!m_dlOutFile.is_open ())
    {
      ...
      m_dlOutFile << "protocol context timestamp imsi cellId rnti lcid packetSize " << std::endl;      
    }
  ...
  m_dlOutFile << "DlTxPDU " << Simulator::Now () << " " << imsi << " " << cellId << " "
              << rnti << " " << (uint32_t) lcid << " " << packetSize << " " << std::endl;
                                      ...
                                      uint64_t delay)
{
  ...
  if (!m_ulOutFile.is_open ())
    {
      NS_LOG_INFO ("New m_ulOutFile criated : " << GetUlOutputFilename ().c_str ());
      m_ulOutFile.open (GetUlOutputFilename ().c_str (), std::ios_base::app);
      m_ulOutFile << "protocol context timestamp imsi cellId rnti lcid packetSize delay" << std::endl;      
    }
  else
    {
      NS_LOG_INFO ("m_ulOutFile already criated : " << GetUlOutputFilename ().c_str () << 
                     ", m_protocolType = " <<  m_protocolType);      
    }  
  ...
  m_ulOutFile << "UlRxPDU " << Simulator::Now () << " " << imsi << " " << cellId << " "
...
NrBearerStatsCalculator::DlRxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, uint32_t packetSize, uint64_t delay)
{
  ...
  if (!m_dlOutFile.is_open ())
    {
      NS_LOG_INFO ("New m_dlOutFile criated : " << GetDlOutputFilename ().c_str ());      
      m_dlOutFile.open (GetDlOutputFilename ().c_str (), std::ios_base::app);
      m_dlOutFile << "protocol context timestamp imsi cellId rnti lcid packetSize delay" << std::endl;
    }
  else
    {
      NS_LOG_INFO ("m_dlOutFile already criated : " << GetDlOutputFilename ().c_str () << 
                     ", m_protocolType = " <<  m_protocolType);      
    }      
  ...
  m_dlOutFile << "DlRxPDU " << Simulator::Now () << " " << imsi << " " << cellId << " "
              << rnti << " " << (uint32_t) lcid << " " << packetSize << " " << delay << std::endl;
...
{
  NS_LOG_INFO ("NrBearerStatsCalculator::GetDlOutputFilename (void) called, m_protocolType = " << m_protocolType);  
  if (m_protocolType == "RLC")
    {
    ...
    { 
      NS_LOG_INFO ("since m_protocolType isnt RLC, outputfile is " << GetDlPdcpOutputFilename ());
...
NrBearerStatsCalculator::SetUlPdcpOutputFilename (std::string outputFilename)
{
  ...
  NS_LOG_INFO ("NrBearerStatsCalculator::SetulPdcpOutputFilename called, outputFilename = " << outputFilename);    
}
...
{
  NS_LOG_INFO ("NrBearerStatsCalculator::SetDlPdcpOutputFilename called, outputFilename = " << outputFilename);
...
{
  NS_LOG_INFO ("GetDlPdcpOutputFilename called  ");
  NS_LOG_INFO ("m_dlPdcpOutputFilename = " << m_dlPdcpOutputFilename);
