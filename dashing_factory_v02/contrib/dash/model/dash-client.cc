...
void TraceCwnd (Ptr<OutputStreamWrapper> stream, uint32_t oldCwnd, uint32_t newCwnd)
{
  *stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << oldCwnd << "\t" << newCwnd << std::endl;
}

void TraceCw (Ptr<OutputStreamWrapper> stream, uint32_t oldCw, uint32_t newCw)
{
  *stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << oldCw << "\t" << newCw << std::endl;
}

void TraceRtt (Ptr<OutputStreamWrapper> stream, Time oldRtt, Time newRtt)
{
...
DashClient::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::DashClient")
          ...
          .AddAttribute ("mytag", "tag ta3ak", UintegerValue (7777),
                         MakeUintegerAccessor (&DashClient::m_tag),
                         MakeUintegerChecker<uint32_t> ())                                                

          // .AddAttribute ("outputDir", 
          //                "output file for stats",
          //                StringValue ("dashhStats.txt"),
          //                MakeStringAccessor (&DashClient::SetOutputDir),
          //                /// MakeStringAccessor (&DashClient::m_outputDir),
          //                MakeStringChecker ())              
      ...
      m_outputDir ("")      
{
  ...
  //                         << std::endl;
  NS_LOG_INFO ("(DashClient::DashClient) m_tag :" <<  m_tag);
  NS_LOG_INFO ("(DashClient::DashClient) bufferSpace :" <<  m_bufferSpace);
  ...
  if (!m_socket)
    {
      ...
      // Trace the TCP congestion window
        std::string cwnd_filename = "cwnd-trace_" + std::to_string(m_id) + "_" + std::to_string(m_tag) +  ".txt";  
        std::string cw_filename = "cw-trace_" + std::to_string(m_id) + "_" + std::to_string(m_tag) +  ".txt";  
        std::string rtt_filename = "rtt-trace_" + std::to_string(m_id) + "_" + std::to_string(m_tag) +  ".txt";  
        NS_LOG_INFO ("cwnd_filename :" << cwnd_filename);  
        NS_LOG_INFO ("rtt_filename :" << rtt_filename);        
        AsciiTraceHelper asciiTraceHelper;

        // RWND : Remote side's flow control window
        Ptr<OutputStreamWrapper> stream0 = asciiTraceHelper.CreateFileStream (cwnd_filename);
        *stream0->GetStream () << "timestamp" << "\t" << "oldcwnd" << "\t" << "newcwnd" << std::endl;
        m_socket->TraceConnectWithoutContext ("RWND", MakeBoundCallback (&TraceCwnd, stream0));

        // CongestionWindow : The TCP connection's congestion window
        Ptr<OutputStreamWrapper> stream1 = asciiTraceHelper.CreateFileStream (cw_filename);
        *stream1->GetStream () << "timestamp" << "\t" << "oldcw" << "\t" << "newcw" << std::endl;
        m_socket->TraceConnectWithoutContext ("CongestionWindow", MakeBoundCallback (&TraceCw, stream1));


        // RTT : Last RTT sample
        Ptr<OutputStreamWrapper> stream2 = asciiTraceHelper.CreateFileStream (rtt_filename);
        *stream2->GetStream () << "timestamp" << "\t" << "oldrtt" << "\t" << "newrtt" << std::endl;
        m_socket->TraceConnectWithoutContext ("RTT", MakeBoundCallback (&TraceRtt, stream2));
...
DashClient::StopApplication (void) // Called at time specified by Stop
{
  ...
  NS_LOG_INFO ("(DashClient::StopApp) m_tag :" <<  m_tag);
  NS_LOG_INFO ("(DashClient::StopApp) bufferSpace :" <<  m_bufferSpace);         
...
void
// DashClient::SetOutputDir (const std::string &outputDir)
DashClient::SetOutputDir (std::string &outputDir)
{
...
DashClient::InitializeCSVs ()
{
  ...
  // std::string filenameDashStatsByNode = BuildFileNameString (m_outputDir, "dashStatsByNode_" + std::to_string(m_id), ".csv");  
  // std::string filenameDashStatsByNode = "dashStatsByNode_" + std::to_string(m_id) + "_" + std::to_string(m_tag) +  ".csv";  
  // m_outDashStatsByNodeFile.open (filenameDashStatsByNode.c_str ()); m_outDashStatsByNodeFile.setf (std::ios_base::fixed);    
  // m_outDashStatsByNodeFile << "timeStamp,Node,newBitRate,oldBitRate,estBitRate,interruptions,InterruptionTime,"
  //                             "bufferTime,deltaBufferTime,estAvgBufferTime,delay_segmentTime,segmentRate,initBuffer" << std::endl;                 
  // m_outDashStatsByNodeFile << "timeStamp,Node,newBitRate,oldBitRate,estBitRate,interTime,T,dT,delay,segmentTime,segmentRate" 

  // std::string filenameSegStatsByNode = BuildFileNameString (m_outputDir, "dashSegmentStatsByNode_" + std::to_string(m_id), ".csv");
  std::string filenameSegStatsByNode = "dashSegmentStatsByNode_" + std::to_string(m_id) + "_" + std::to_string(m_tag) +  ".csv";      
  /// std::string filenameSegStatsByNode = m_outputDir + "dashSegmentStatsByNode_" + std::to_string(m_id) + ".csv";
...
std::string DashClient::BuildFileNameString (std::string directoryName, std::string filePrefix, std::string tag)
{
