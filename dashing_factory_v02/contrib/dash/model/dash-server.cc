...
DashServer::StartApplication () // Called at time specified by Start
{
  ...
  Ptr<Ipv4> node_ipv4 = m_node->GetObject<Ipv4>();                      
  Ipv4Address ipv4Address = node_ipv4->GetAddress (1, 0).GetLocal ();                          

  NS_LOG_INFO ("    ------------------------------------------------------");
  NS_LOG_INFO ("    [" << Simulator::Now () << "] DashServer::StartApplication () ");
  NS_LOG_INFO ("                               at node" << m_node->GetId () << " (i.e. ue" << m_node->GetId () - 12 << ")");            
  NS_LOG_INFO ("                               ipv4 [" << ipv4Address << "]");            
  NS_LOG_INFO ("                               phy address [" << m_node->GetDevice(1)->GetAddress() << "]");            
  NS_LOG_INFO ("                               m_local [" << m_local << "]");            
  NS_LOG_INFO ("");           
  // Create the socket if not already
  if (!m_socket)
    {
      NS_LOG_INFO ("                               new socket, create, bind and listen ...");                  
  ...
  NS_LOG_INFO ("    [" << Simulator::Now () << "] DashServer::StartApplication DONE ");
  NS_LOG_INFO ("    ------------------------------------------------------");      
  NS_LOG_INFO ("");             
}
...
DashServer::HandleRead (Ptr<Socket> socket)
{
  ...
  NS_LOG_INFO ("--------------------------------------------------");
  NS_LOG_INFO (Simulator::Now () << "[" << m_local << "] DashServer::HandleRead () ");
  NS_LOG_INFO ("                                         socket from Node" << socket->GetNode ()->GetId ());   
  NS_LOG_INFO ("                                         (Called when a request is received)");     
  NS_LOG_INFO ("");     

  Ptr<Packet> packet;
  Address from;

  NS_LOG_INFO ("                                         while ((packet = socket->RecvFrom (from))) ->");               
  while ((packet = socket->RecvFrom (from)))
    {
      m_totalRx += packet->GetSize ();

      NS_LOG_INFO ("                                             // recall DashServer::HandleRead ()");
      if (m_pending_packet == NULL)
        {
          NS_LOG_INFO ("                                                m_pending_packet is null ==>  m_pending_packet = packet");
          m_pending_packet = packet;
        }
      else
        {
          NS_LOG_INFO ("                                                m_pending_packet is NOT null ==>  m_pending_packet->AddAtEnd (packet);");
      ...
      while (m_pending_packet->GetSize () >= header.GetSerializedSize ())
        {
          ...
          if (header.GetMessageType () == HTTP_REQUEST)
            {
              NS_LOG_INFO ("                     inside (server) HandleRead (), ==> sendSegment ... ");
      ...
      if (InetSocketAddress::IsMatchingType (from))
        {
          NS_LOG_INFO ("                     inside HandleRead (), At time " << Simulator::Now ().GetSeconds () << "s packet sink received "
      ...
      NS_LOG_INFO ("                     inside HandleRead (), m_rxTrace (packet, from) to be called ... ");          
      m_rxTrace (packet, from);
    }
  NS_LOG_INFO (Simulator::Now () << "[" << m_local << "] DashServer::HandleRead () DONE ");
  NS_LOG_INFO ("--------------------------------------------------");        
}
...
DashServer::HandleAccept (Ptr<Socket> s, const Address &from)
{
  ...
  NS_LOG_INFO ("--------------------------------------------------");
  NS_LOG_INFO (Simulator::Now () << "[" << m_local << "] DashServer::HandleAccept ()  ");
  NS_LOG_INFO ("                                         socket from Node" << s->GetNode ());  
  NS_LOG_INFO ("                                         address " << from);  
  NS_LOG_INFO ("                                         // called hen a new connection is accepted");  

  s->SetRecvCallback (MakeCallback (&DashServer::HandleRead, this));
  s->SetSendCallback (MakeCallback (&DashServer::DataSend, this));
  m_socketList.push_back (s);

  NS_LOG_INFO (Simulator::Now () << "[" << m_local << "] DashServer::HandleAccept () DONE ");
  NS_LOG_INFO ("--------------------------------------------------");    
}
...
DashServer::DataSend (Ptr<Socket> socket, uint32_t)
{
  ...
  NS_LOG_INFO ("    ------------------------------------------------------");
  NS_LOG_INFO ("    [" << Simulator::Now () << "] DashServer::DataSend () ");
  NS_LOG_INFO ("                     socket from Node" << socket->GetNode ()->GetId ());  
  NS_LOG_INFO ("                     (called when a new segment is transmitted)");
  NS_LOG_INFO ("                     (or when new space is aveilable in the buffer)");
  NS_LOG_INFO ("");      
  ...
  while (!m_queues[socket].empty ())
    {
      ...
      if ((bytes = socket->Send (frame)) < frame->GetSize ())
        ...
        {
          NS_LOG_INFO ("                     inside DataSend (), Just sent " << frame->GetSerializedSize () << " " << frame->GetSize ());
          /// NS_LOG_INFO ("Just sent " << frame->GetSerializedSize () << " " << frame->GetSize ());
          //socket->Send(Create<Packet>(0));
        }
    }

  NS_LOG_INFO ("    [" << Simulator::Now () << "] DashServer::DataSend () DONE");
  NS_LOG_INFO ("    ------------------------------------------------------");      
  NS_LOG_INFO ("");         
}
                         ...
                         Ptr<Socket> socket)
{
  ...
  NS_LOG_INFO ("    ------------------------------------------------------");
  NS_LOG_INFO ("    [" << Simulator::Now () << "] DashServer::SendSegment () ");
  NS_LOG_INFO ("                     socket from Node" << socket->GetNode ()->GetId ());  
  NS_LOG_INFO ("                     // sends the segment back to the client)");
  NS_LOG_INFO ("");         
  ...
  for (uint32_t f_id = 0; f_id < MPEG_FRAMES_PER_SEGMENT; f_id++)
    {
      ...
      NS_LOG_INFO ("SENDING PACKET "
                   ...
                   << " size=" << mpeg_header.GetSize () << " avg=" << avg_packetsize); // 50fps 
      NS_LOG_INFO ("               segment_id = (" << segment_id  << ") "
                   << ", frameId = (" << f_id << ") "
                   << ", playback=(MilliSeconds ((( " << (f_id + (segment_id * MPEG_FRAMES_PER_SEGMENT)) * MPEG_TIME_BETWEEN_FRAMES 
                   << "))"); //50 fps

      m_queues[socket].push_back (*frame);
    }
  NS_LOG_INFO ("                     inside SendSegment (), DataSend (socket, 0) to be called ... ");              
  DataSend (socket, 0);

  NS_LOG_INFO ("    [" << Simulator::Now () << "] DashServer::SendSegment () DONE ");
  NS_LOG_INFO ("    ------------------------------------------------------");      
  NS_LOG_INFO ("");                
}
