...
public:
  ...
  /**
     * \brief Initialize CSV files.
     */
  void InitializeCSVs ();

  /**
   * \brief Output some statistics in csv format.
   */
  std::string GetStatsCsv ();
...
private:
  ...
  Time m_window;
  Time m_segmentFetchTime; // m_segmentFetchTime = now - m_requestTime
  Time m_totalSegFetchTime; // cumulatif m_segmentFetchTime
  bool m_RequestPending = false; // So that we don't request the same segment repeatedly

  uint32_t m_receivedSegments;
  std::ofstream m_outDashStatsByNodeFile;    //!< the output file stream for Dash stats
  std::ofstream m_outSegStatsByNodeFile;     //!< the output file stream for segments stats per node
