...
public:
  ...
  uint32_t m_minRate;
  uint32_t m_maxRate;
  uint32_t m_minminRate;
  uint32_t m_framesPlayed;
  FrameBuffer m_frameBuffer;

  double m_init_buff; // initial buffering time : period between starting time of loading 
                      // the vid and the time of playing it
  bool m_firstPlay;   // Do we over started playing a video ? (default : False)
  Time m_firstBuff;   // Time of the first buffering event
