...
// INDOOR FACTORY - SPARSE CLUTTER LOW - iTEAM UPV ---------------------------------- //
...
ThreeGppIndoorFactorySLChannelConditionModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ThreeGppIndoorFactorySLChannelConditionModel")
    ...
    .AddAttribute ("r_density_SL", "The minimums density",
                   DoubleValue (0),
                   MakeDoubleAccessor (&ThreeGppIndoorFactorySLChannelConditionModel::r_density_SL),
...

{
  ...
  if (h_BS >= 10)
	...
	//to compute clutter density according to clutter distance

	double dclutter = 0.0;
	if (r_density_SL < 0.40)
		{
		dclutter = 10.0;
		}

	else if (r_density_SL > 0.40)
		{
		dclutter = 2.0;
		}

	plos = exp (- distance2D / (-(dclutter / log (1.0 - r_density_SL))));
	}
  return plos;
}

  // INDOOR FACTORY - DENSE CLUTTER LOW - iTEAM UPV---------------------------------- //
  ...
  ThreeGppIndoorFactoryDLChannelConditionModel::GetTypeId (void)
  {
    static TypeId tid = TypeId ("ns3::ThreeGppIndoorFactoryDLChannelConditionModel")
	  ...
	  .AddAttribute ("r_density_DL", "The minimums density",
	                   DoubleValue (0),
	                   MakeDoubleAccessor (&ThreeGppIndoorFactoryDLChannelConditionModel::r_density_DL),
	                   MakeDoubleChecker<double> ())
	;
    return tid;
  }
                                                              ...
                                                              Ptr<const MobilityModel> b) const
  {
    ...
    // retrieve h_BS, it should be < 10 m
    double h_BS = std::max (a->GetPosition ().z, b->GetPosition ().z);
    if (h_BS > 10.0)
      {
        NS_LOG_WARN ("The LOS probability was derived assuming BS antenna heights of <10 m (see TR 38.901, Table 7.4.2-1)");
    ...
    if (h_BS > 10.0)
  	...
  	else
  	 {
  		...
  		if (r_density_DL < 0.40)
  			{
  			dclutter = 10.0;
  			}

  		else if (r_density_DL >= 0.40)
  			{
  			dclutter = 2.0;
  			}
  		plos = exp (- distance2D / (-(dclutter / log (1.0 - r_density_DL))));
  	 }
  return plos;
  }
  ...
  // INDOOR FACTORY - SPARSE CLUTTER HIGHT - iTEAM UPV ---------------------------------- //
  ...
  ThreeGppIndoorFactorySHChannelConditionModel::GetTypeId (void)
  {
    static TypeId tid = TypeId ("ns3::ThreeGppIndoorFactorySHChannelConditionModel")
	  ...
	  .AddAttribute ("r_density_SH", "The minimums density",
	                   DoubleValue (0),
	                   MakeDoubleAccessor (&ThreeGppIndoorFactorySHChannelConditionModel::r_density_SH),
	                   MakeDoubleChecker<double> ())
		.AddAttribute ("hc_SH", "hegith of clutter",
					  DoubleValue (0),
					  MakeDoubleAccessor (&ThreeGppIndoorFactorySHChannelConditionModel::hc_SH),
					  MakeDoubleChecker<double> ())
    ;
    return tid;
  }
...

  {
    ...
    if (h_BS < hc_SH)
      {
        NS_LOG_WARN ("The LOS probability was derived assuming BS antenna heights is > heigh clutter (see TR 38.901, Table 7.2-4)");
      }
    //The effective clutter height should be < ceiling height (0 - 10 m)

    if (hc_SH > 10.0)
      {
    	NS_LOG_WARN ("hc does not apply, Table 7.2-4");
      }
    //to compute h_UT as others scenarios, it should be smaller than clutter height
    double h_UT = std::min (a->GetPosition ().z, b->GetPosition ().z);

    if (h_UT > hc_SH)
      {
    	NS_LOG_WARN ("The height of the UT should be smaller than hc (see TR 38.901, Table 7.2-4)");
      }
    ...
    if (r_density_SH <= 0.0)
      {
  	...
  	else
  	  {
  		...
  		if (r_density_SH < 0.40)
  		  {
  			dclutter = 10.0;
  		  }

  		else if (r_density_SH > 0.40)
  		  {
  			dclutter = 2.0;
  		  }
  		plos = exp (- distance2D / (-(dclutter / log (1.0 - r_density_SH)) * ((h_BS - h_UT) / (hc_SH - h_UT))));
  	  }
  return plos;
  }

  // INDOOR FACTORY - DENSE CLUTTER HIGHT - iTEAM UPV---------------------------------- //
  ...
  ThreeGppIndoorFactoryDHChannelConditionModel::GetTypeId (void)
  {
    static TypeId tid = TypeId ("ns3::ThreeGppIndoorFactoryDHChannelConditionModel")
	  ...
	  .AddAttribute ("r_density_DH", "The minimums density",
		                   DoubleValue (0),
		                   MakeDoubleAccessor (&ThreeGppIndoorFactoryDHChannelConditionModel::r_density_DH),
		                   MakeDoubleChecker<double> ())
	  .AddAttribute ("hc_DH", "height of clutter",
						  DoubleValue (0),
						  MakeDoubleAccessor (&ThreeGppIndoorFactoryDHChannelConditionModel::hc_DH),
						  MakeDoubleChecker<double> ())
  ;
  return tid;
  }
...

  {
    ...
    if (h_BS < hc_DH)
      {
       NS_LOG_WARN ("The LOS probability was derived assuming BS antenna heights is > height clutter (see TR 38.901, Table 7.2-4)");
      }

    //Compute hc which is effective clutter height. < ceiling height, 0 - 10 m
    if (hc_DH > 10.0)
      {
       NS_LOG_WARN ("clutter height does not apply, Table 7.2-4");
      }
    //Compute h_UT as others scenarios, I guess it should be smaller than hc
    double h_UT = std::min (a->GetPosition ().z, b->GetPosition ().z);
    ...
    if (r_density_DH <= 0.0)
      ...
      if (r_density_DH < 0.40)
      	{
    	  dclutter = 10.0;
      	}

      else if (r_density_DH >= 0.40)
      	{
    	  dclutter = 2.0;
      	}
      	plos = exp (- distance2D / (-(dclutter / log (1.0 - r_density_DH)) * ((h_BS - h_UT) / (hc_DH - h_UT))));
      }
    return plos;
      }
