...
def topology():
    ...
    linkopts = dict(delay='18.2812ms', jitter="0.1324")
    # net.addLink(s1, s2, 100, 1, **linkopts)
...
def godash_client(num , host, algo, Band_trace ):
    ...
    ## ./goDASH --config ../config/con/configure.json \
    print ("start godash client ...")
    print ("    num=", num ,", host=", host,"algo=", algo, ", trace=", Band_trace)
    print ("    let try to ping 192.168.0.150")
    print (num.cmd('ping -c 2 192.168.0.150 >/home/dash/Demo/ping.txt'))
    print (num.cmd('cd /home/dash/Demo/testbed/goDash/DashApp/src/goDASH/ && \
                   ./goDASH --config /home/dash/Demo/testbed/goDash/DashApp/src/config/con/configure.json \
                   >/home/dash/Demo/Data/Raw/total_host_' \
                   + str(host) + '_algo_'+str(algo)+ '_5g_trace_'+str(Band_trace)+'/' + str(num) + '_algo_'+str(algo)+ '_5g_trace_'+str(Band_trace)+'.txt &&\
                      echo stop streaming_' + str(num) ))
...
def  tc(col_no):
     ...
     # os.system('sudo ./bwc-new.sh %d'%(col_no))
     os.system('sudo ./bwc-raw.sh %d'%(col_no))
